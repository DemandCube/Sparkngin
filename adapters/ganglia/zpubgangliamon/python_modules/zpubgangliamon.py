...
def metric_init(params):
    ...
    zpubgangliamon.py. If *params* contains a `groups` key, its value is used
    as the group name in Ganglia (default 'ZeroMQ').
...
def self_test():
    ...
    socket.bind("ipc://localhost:5656")
    pub_thread = threading.Thread(target='start_pub_server', args=socket)
    ...
    print params.get
    if not params:
        print 'Usage: %s METRIC_NAME PUB_URI [METRIC_NAME PUB_URI, ...]' % sys.argv[0]
        print 'Example: %s zmq_published_msg tcp://localhost:8006' % sys.argv[0]
        sys.exit(1)
